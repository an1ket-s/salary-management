...
const ALLOWED_SORT = new Set(["firstName", "lastName", "salary", "joiningDate", "createdAt"]);
...
export const employeesRepository = {
  async findMany(filters: EmployeeFilters) {
    ...
    const sortBy = ALLOWED_SORT.has(filters.sortBy ?? "") ? filters.sortBy! : "createdAt";
  ...
  async getMeta() {
    const [countries, departments, roles] = await prisma.$transaction([
      ...
      prisma.employee.findMany({ select: { role: true },       distinct: ["role"],       orderBy: { role:       "asc" } }),
    ]);
    return {
